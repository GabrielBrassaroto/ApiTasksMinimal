...
app.MapGet("phrases", async async=> await new HttpClient().GetStreamAsync("https://ron-swanson-quotes-herokuapp.com/v2/quotes"));


app.MapGet("/tasks", async (AppDbContext db) => { return await db.Tasks.ToListAsync(); });

app.MapGet("/tasks/{id}", async (int id, 
    AppDbContext db) => await
  db.Tasks.FindAsync(id) is Task task ? Results.Ok(task): Results.NotFound());

app.MapGet("/tasks/completed", async (AppDbContext db) => 
    await db.Tasks.Where(x => x.IsCompleted).ToListAsync());
